# This file is a part of DF_SLAM.
#
# DF_SLAM is a NeRF-based SLAM system. It utilizes Neural Radiance Fields (NeRF)
# to perform Simultaneous Localization and Mapping (SLAM) in real-time.
# This software is the implementation of the paper "DF_SLAM: Efficient Dense SLAM
...
class Renderer(object):
    ...
    """
    def __init__(self, cfg, df_slam, ray_batch_size=10000):
        ...
        self.bound = df_slam.bound.to(df_slam.device, non_blocking=True)
    ...
    def render_img(self, model, c2w, device, gt_depth=None):
        """
        ...
        with torch.no_grad():
            ...
            for i in range(0, rays_d.shape[0], ray_batch_size):
                ...
                if gt_depth is None:
                    color, depth, _, _, _ = self.render_ray(rays_o_batch, rays_d_batch, model, gt_depth=None)
                else:
                    gt_depth_batch = gt_depth[i:i+ray_batch_size]
                    color, depth, _, _, _ = self.render_ray(rays_o_batch, rays_d_batch, model, gt_depth_batch)
    ...
    def render_ray(self, rays_o, rays_d, model, gt_depth=None):

        rays = torch.cat([rays_o, rays_d], dim=-1)
        rays = rays.to(self.device)

        rgb_map, depth_map, sdf, z_vals, sigma = model.run_network(rays, gt_depth)

        return rgb_map, depth_map, sdf, z_vals, sigma
