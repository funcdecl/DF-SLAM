...
class Mesher(object):
    ...
    """
    def __init__(self, cfg, args, df_slam, points_batch_size=500000, ray_batch_size=100000):
    ...
    def eval_points(self, p, model):
        ...
        for pi in p_split:
            ...
            feats_g, feats_c = model.get_coding(pi)
            feat_g = model.linear_mat(feats_g)

            sdf = feat_g[..., 0]
            sdf = sdf.unsqueeze(-1)

            rgbs = model.renderModule(feats_c)

            ret = torch.cat([rgbs, sdf], dim=-1)
    ...
    def get_mesh(self, mesh_out_file, keyframe_dict, model, device='cuda:0', color=True):
        """
        ...
        with torch.no_grad():
            ...
            for i, pnts in enumerate(torch.split(points, self.points_batch_size, dim=0)):
                z.append(self.eval_points(pnts.to(device), model).cpu().numpy()[:, -1])
            ...
            if color:
                ...
                z = []
                for i, pnts in enumerate(torch.split(points, self.points_batch_size, dim=0)):
                    z_color = self.eval_points(pnts.to(device).float(), model).cpu()[..., :3]
                    z.append(z_color)
                z = torch.cat(z, dim=0)
