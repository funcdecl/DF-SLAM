...
class Frame_Visualizer(object):
    ...
    def save_imgs(self, idx, iter, gt_depth, gt_color, c2w_or_camera_tensor, model):
        """
        ...
        with torch.no_grad():
            if (idx % self.freq == 0) and (iter % self.inside_freq == 0):
                ...
                depth, color = self.renderer.render_img(model, c2w, self.device, gt_depth=gt_depth)
                ...
                psnr_depth = calculate_psnr(gt_depth, depth)
                psnr_color = calculate_psnr(gt_color, color)

                print(f'PSNR of Depth: {psnr_depth.item()}, PSNR of Color: {psnr_color.item()}')
