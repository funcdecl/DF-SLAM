...
def evaluate_ate(first_list, second_list, plot="", _args=""):
    ...
    with open('metrics_output.txt', 'a') as file:  # 'a' mode appends to the file
        file.write(f'absolute_translational_error.rmse: {numpy.sqrt(numpy.dot(trans_error, trans_error) / len(trans_error))}\n')
        file.write(f'absolute_translational_error.mean: {numpy.mean(trans_error)}\n')
